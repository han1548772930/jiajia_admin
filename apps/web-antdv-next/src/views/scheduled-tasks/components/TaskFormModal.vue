...
<template>
  <Modal :title="taskForm.jobState.title" class="w-[90%]">
    <Form ref="formRef" :model="taskForm.values" layout="vertical" class="h-full w-full" :validate-trigger="[]">
      <div class="w-full overflow-y-auto p-4">
        <div class="flex w-full flex-col gap-4 sm:flex-row">
          <div class="w-full space-y-3 sm:w-96 sm:shrink-0">
            <Divider>基础信息</Divider>

            <FormItem label="任务名称" name="jobState.jobName" :rules="[{ required: true, message: '请输入任务名称' }]"
              class="mb-4">
              <Input v-model:value="taskForm.values.jobState.jobName"
                :disabled="taskForm.values.jobState.type === JobOperateType.EDIT" />
            </FormItem>

            <FormItem label="所属分组" name="jobState.groupId" :rules="[{ required: true, message: '请选择所属分组' }]"
              class="mb-4">
              <Select v-model:value="groupIdModel" :options="groupOptions" show-search option-filter-prop="label"
                placeholder="请选择" />
            </FormItem>

            <FormItem label="任务类型" name="jobState.jobType"
              :rules="[{ required: taskForm.values.jobState.jobStatus, message: '请选择任务类型' }]" class="mb-4">
              <Select v-model:value="taskForm.values.jobState.jobType" :options="jobTypeOptions" placeholder="请选择" />
            </FormItem>

            <FormItem label="任务状态" name="jobState.jobStatus" class="mb-4">
              <Switch v-model:checked="taskForm.values.jobState.jobStatus" checked-children="启用"
            ...
            <FormItem label="开始时间" name="jobState.startTime"
              :rules="[{ required: taskForm.values.jobState.jobStatus, message: '请选择开始时间' }]" class="mb-4">
              <DatePicker v-model:value="taskForm.values.jobState.startTime" show-time class="w-full" />
            </FormItem>

            <FormItem label="结束时间" name="jobState.endTime" class="mb-4">
              <DatePicker v-model:value="taskForm.values.jobState.endTime" show-time class="w-full" />
            </FormItem>

            <FormItem v-if="taskForm.values.jobState.jobType === JobType.AutoReport" label="循环语句"
              name="jobState.loopSQL" :rules="[{ required: taskForm.values.jobState.jobStatus, message: '请输入循环语句' }]"
              class="mb-4">
              <TextArea v-model:value="taskForm.values.jobState.loopSQL" :rows="3" />
            </FormItem>
          </div>
